...
public enum ConnectionState {
    /**
     *  CONNECTING : 初始化 connect 信息的时候（后面会建立一个 SocketChannel 并注册到 Selector 上）
     *  DISCONNECTED： 初始化 connect 失败（建立 SocketChanel 并注册时 出现异常）
     *  CHECKING_API_VERSIONS：SocketChannel 成功 建立 Connect (后面会向 kafka server 发送一个 request)
     *  READY: SocketChannel 发送的 ApiRequest 被成功 接收
     */
