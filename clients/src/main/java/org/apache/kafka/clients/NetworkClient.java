...
public class NetworkClient implements KafkaClient {
    ...
    // key： node id , 每新建一个 Connection 都会 添加到这个里面
    // 用于向 Kafka server 发送 MetaData 更新请求, 建立完 发送请求后 就会移除掉
...
//        System.out.println(connectionStates.isReady(node, now) +"  | " + (selector.isChannelReady(node)) +"  | " + (inFlightRequests.canSendMore(node)));
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now) {
        ...
        AbstractRequest.Builder<?> builder = clientRequest.requestBuilder();
                try {
                    ...
                    // versionInfo = null , 发送 ApiVersionRequest , version = Schema.length - 1
                    if (versionInfo == null) {
                        ...
                        // ApiKeys.latestVersion (用于处理版本问题)
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now, AbstractRequest request) {
        ...
        InFlightRequest inFlightRequest = new InFlightRequest(
                ...
                send,
                now);
        // 将待发送 Request 的数据封装为 InFlightRequest，并添加到 <nodeID,Deque> 中
        this.inFlightRequests.add(inFlightRequest);
        // 将 Send 添加到 kafkaChannel 中，并新增 SelectionKey 的 OP_WRITE 事件
        selector.send(send);
    }
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        // 如果 MetaData 需要更新，那么获取一个 node 信息，并确保 该 node 的 Connection 是可用的
        // 真正处理 MetaData 更新的地方
        long metadataTimeout = metadataUpdater.maybeUpdate(now);
        try {
            // 调用 java nio selector 处理处于就绪状态的 通道
        ...
        // 处理 SocketChannel 新建立的 Connection
        handleConnections();
        // 处理新建立连接的 API
    ...
    /**
     *
     * 根据已存在的 connection 获取 该 connection 的 对应的 node
     * 如果没有 connection 的话，随机选择一个 node
    ...
    public Node leastLoadedNode(long now) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            // 该 node 节点 发送的 request 以及 接收到的 response 个数
            int currInflight = this.inFlightRequests.count(node.idString());
            // node 节点是否可以发送 requests 请求
...
//            System.out.println("deal recieve >>>>>>>");
            ...
            if (req.isInternalRequest && body instanceof MetadataResponse) {
                System.out.println("metaData 更新完毕");
     ...
     *
     * 处理新建立的连接，确保该 connection 能够发送请求
     */
    private void handleConnections() {
        for (String node : this.selector.connected()) {
//            System.out.println("handleConnections: " + node);
...
//        System.out.println("Initiating connection to node : " + node);
        String nodeConnectionId = node.idString();
        try {
            log.debug("Initiating connection to node {}", node);
            // 将 node 信息添加到 ClusterConnectionStates::Map<String, NodeConnectionState> nodeState;
            this.connectionStates.connecting(nodeConnectionId, now);
            /**
             *  1. 建立 SocketChannel
             *  2. 将 Socketchannel 注册到 java nio selector 上，并绑定 OP_CONNECT 事件
             *  3. 创建 KafakChannel 对象绑定到 SocketChannel 的 SelectionKey 上
             *  4. 将 KafkaChannel 缓存到 本地的   Map<String, KafkaChannel> channels
             */
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        //  已经发送 MetaData Request 请求，但还没有 接收到 Response ，此时才为 true
...
//            Instant specificTime = Instant.ofEpochMilli(now);
            // 需要更新时 =  0，否则 > 0
            long timeToNextMetadataUpdate = metadata.timeToNextUpdate(now);
            // metadataFetchInProgress 默认为 false
            ...
            // 获取 request+response 最少的 node （负载均衡）, 并且 该 node 没有建立过连接
            Node node = leastLoadedNode(now);
            if (node == null) {
                log.debug("Give up sending metadata request since no node is available");
                return reconnectBackoffMs;
            }
//            System.out.println("seelct node : " +node);
        ...
        private long maybeUpdate(long now, Node node) {
             ...
             *  2. 该 node 节点没有已发送但是未接收的数据
             *  3. kafkaChannel 处于 Ready 状态
             */
...
//                    System.out.println(" not update all");
                    metadataRequest = new MetadataRequest.Builder(new ArrayList<>(metadata.topics()),
                            metadata.allowAutoTopicCreation());
                }
                System.out.println("构建 metaData 更新请求");
            ...
            // 如果 node 不存在 对应的 Map<String, NodeConnectionState> nodeState ，返回 true
            if (connectionStates.canConnect(nodeConnectionId, now)) {
                // we don't have a connection to this node right now, make one
                log.debug("Initialize connection to node {} for sending metadata request", node);
//                System.out.println("Initialize connection");
                // 根据 node 信息，初始化 Connection
                initiateConnect(node, now);
                return reconnectBackoffMs;
            }
