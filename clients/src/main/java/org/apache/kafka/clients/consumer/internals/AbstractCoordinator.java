...
public abstract class AbstractCoordinator implements Closeable {
    ...
    // JoinGroupRequest 请求发送成功后为 false
    private boolean rejoinNeeded = true;
    private boolean needsJoinPrepare = true;
    private MemberState state = MemberState.UNJOINED;
    private RequestFuture<ByteBuffer> joinFuture = null;
    // find Coordinator Request 后获取返回节点
    private Node coordinator = null;
    // Join Group Request 成功后进行实例化
    private Generation generation = Generation.NO_GENERATION;

    // 将 FindCoordinatorRequest + node 信息缓存到 UnsentRequests 类里面、
    // FindCoordinatorRequest 的回调实现： RequestFutureCompletionHandler
    // RequestFutureCompletionHandler 中缓存的是 RequestFuture<ClientResponse>
    // RequestFuture<Void> 被 RequestFuture<ClientResponse> 的监听器所引用
    // RequestFuture<ClientResponse> 的监听器 是由 FindCoordinatorResponseHandler 实现的
    // RequestFuture<Void> = new RequestFuture()
    ...
    protected synchronized boolean ensureCoordinatorReady(final Timer timer) {
        ...
        do {
            ...
            // RequestFutureCompletionHandler 中缓存的是 RequestFuture<ClientResponse>
            // RequestFuture<Void> 被 RequestFuture<ClientResponse> 的监听器所引用
            // RequestFuture<ClientResponse> 的监听器 是由 FindCoordinatorResponseHandler 实现
            // RequestFuture<Void> = new RequestFuture() 用于阻塞发送 FindCoordinatorRequest 请求
            final RequestFuture<Void> future = lookupCoordinator();

            //  初始化 node channel ，发起 FindCoordinatorRequest 请求
            //  根据返回的 node 信息，作为 coordinator
            //  准备好 coordinator 节点
    ...
    protected synchronized RequestFuture<Void> lookupCoordinator() {
        if (findCoordinatorFuture == null) {
            ...
            // 通过 NetWorkClient 配置的 bootstrap.servers 获取一个节点
            Node node = this.client.leastLoadedNode();
            if (node == null) {
                log.debug("No broker available to send FindCoordinator request");
                return RequestFuture.noBrokersAvailable();
            } else
                // 构件 FindCoordinatorRequest，准备发送请求
     ...
     * of the client. This must be called periodically(定期) after joining with {@link #ensureActiveGroup()}
    ...
    boolean ensureActiveGroup(final Timer timer) {
        ...
        // 每一次都需要确认 coordinator 是否准备就绪，因为 coordinator 节点可能会丢失连接
    ...
    boolean joinGroupIfNeeded(final Timer timer) {
        while (rejoinNeededOrPending()) {
            ...
            // still in progress.
            if (needsJoinPrepare) {
                // ConsumerRebalanceListener.onPartitionsRevoked (当没有实现监听器，默认没有操作)
            ...
            /**
             * 如果没有 group.id 会报 InvalidGroupIdException
             */
            if (future.succeeded()) {
    ...
    private synchronized RequestFuture<ByteBuffer> initiateJoinGroup() {
        ...
        if (joinFuture == null) {
            ...
            // 构建 JoinGroupRequest 请求
            // 将 JoinGroupRequest + coordinator 信息缓存到 UnsentRequests 类里面、
            // JoinGroupRequest 的回调实现： RequestFutureCompletionHandler
            // RequestFutureCompletionHandler 中缓存的是 RequestFuture<ClientResponse>
            // RequestFuture<ClientResponse> 的监听器 是由 JoinGroupResponseHandler 实现的
            // RequestFuture<ByteBuffer> = new RequestFuture()
            // RequestFuture<ByteBuffer> 用于阻塞等待 请求完成
            // 在监听器
            joinFuture = sendJoinGroupRequest();


            // RequestFuture<ByteBuffer> 添加监听事件
    ...
    RequestFuture<ByteBuffer> sendJoinGroupRequest() {
        ...
        JoinGroupRequest.Builder requestBuilder = new JoinGroupRequest.Builder(
                ...
                this.generation.memberId,   // memberId 默认为 空
                protocolType(),
    ...
    private RequestFuture<Void> sendFindCoordinatorRequest(Node node) {
        ...
        return client.send(node, requestBuilder)  //RequestFuture<ClientResponse>
                     .compose(new FindCoordinatorResponseHandler()); //RequestFuture<Void>
    }
