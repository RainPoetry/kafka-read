...
public final class ConsumerCoordinator extends AbstractCoordinator {
    ...
    private PartitionAssignor lookupAssignor(String name) {
        System.out.println("this.assignors: " + this.assignors.size());
    ...
    public boolean poll(Timer timer) {
        // OffsetCommitCompletion 成功后调用  onComplete 方法
        invokeCompletedOffsetCommitCallbacks();

        // subscribe: true
        // assign : false
        // 是否自动分配 partition， assign 中已经指定了分区
        if (subscriptions.partitionsAutoAssigned()) {
            // Always update the heartbeat last poll time so that the heartbeat thread does not leave the
            // group proactively(主动) due to application inactivity(活动) even if (say) the coordinator cannot be found.
            pollHeartbeat(timer.currentTimeMs());
            // 发送 FindCoordinatorRequest 请求
            //  ensureCoordinatorReady ：获取 coordinator 节点 ,
            // coordinator 节点 = __consumer_offsets 主题 所在 分区的 leader 节点
            // ( 分区 = groupId.hashCode() % 50)
            ...
            if (rejoinNeededOrPending()) {
                ...
                // 开启心跳检测线程 , 发送 HeartBeat 请求到 Server 端
                // 将 consumer 加入到 consumer Group
