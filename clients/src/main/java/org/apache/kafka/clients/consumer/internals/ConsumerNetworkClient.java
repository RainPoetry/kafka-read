...
public class ConsumerNetworkClient implements Closeable {
	...
	// 当请求发送完毕后，将 RequestFutureCompletionHandler 新增进来
    ...
    public boolean poll(RequestFuture<?> future, Timer timer) {
        // RequestFuture 有一个监听器（FindCoordinatorResponseHandler）
        // 这个监听器的 onSuccess 被调用
        do {
            poll(timer, future);
    ...
    public void poll(Timer timer, PollCondition pollCondition, boolean disableWakeup) {
        /**
         *
         *  这里的  FutureRequest 不是 RequestFutureCompletionHandler 内部的 FutureRequest<ClientResponse>
         *      而是通过 compose 返回的 FutureRequest<?>
         */


        // there may be handlers which need to be invoked if we woke up the previous call to poll
        // 用于跳出当前循环
        // 数据发送完毕后会将 RequestFutureCompletionHandler  缓存起来
        // RequestFutureCompletionHandler 中的 RequestFuture<ClientResponse> 通过 complete 调用 监听器 事件
        // 监听器  通过调用 FutureRequest.complete(value) 将值传给 RequestFuture ，从而改变 result 值跳出死循环
        // 执行监听器事件：
            // FindCoordinatorResponseHandler：
            // （监听器的实现类）FindCoordinatorResponseHandler 调用 RequestFuture<Void>.complete(null) ，传递一个空值

            // JoinGroupResponseHandler：
            /**
             * 监听器事件：
             *      发送 SyncGroupRequest 请求
             *      等待 SyncGroupRequest 完毕
             *
             */

            // SyncGroupRequest：
            /**
             * 1. 监听器事件：执行 FutureRequest.complete
             *
             * 2. 在 FutureRequest 的监听器中 执行 FutureRequest.complete
             *  (在 JoinGroupResponseHandler 的监听器 将 FutureRequest(被阻塞的 FutureRequest) 传了进来)
             *
             */
        ...
        try {
            ...
            // 从 unsent 队列中获取 发送请求
            // 根据发送请求的 Node , 初始化 Channel
            // 通过 Channel，做好数据发送的一切准备工作
            long pollDelayMs = trySend(timer.currentTimeMs());

            // check whether the poll is still needed by the caller. Note that if the expected completion
            // condition becomes satisfied after the call to shouldBlock() (because of a fired completion
            // handler), the client will be woken up.
            // NetWorkClient 建立Channel 、读取 Channel 、写入 Channel ，实现与 Server 端的交互
    ...
    private void firePendingCompletedRequests() {
        ...
        for (;;) {
            // 获取发送完毕的 request 请求的 RequestFutureCompletionHandler
            RequestFutureCompletionHandler completionHandler = pendingCompletion.poll();
//            System.out.println("completionHandler = " + completionHandler);
            if (completionHandler == null)
                break;
            // 在 FutureRequest 中调用所有的监听器
