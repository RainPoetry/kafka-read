...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            if (metadata != null) {
                ...
                // 初始化 metadata、Cluster
                this.metadata = new Metadata(retryBackoffMs, config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG),
                    true, true, clusterResourceListeners);
                // 将配置的  bootstrap.servers  信息更新到 Cluter 中
            ...
            // 启动 传入到 KafkaThread 中 的 Sender 线程
            this.ioThread.start();
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long maxWaitMs) throws InterruptedException {
        ...
        // metadata 初始化的时候会 构建 没有任何信息的 cluster 实例对象
        Cluster cluster = metadata.fetch(); //

        if (cluster.invalidTopics().contains(topic))
            throw new InvalidTopicException(topic);

        // 将 topic 添加到 Metadata 的 Map<String, Long> topics 中，如果原来的 Metadata 中不存在该 topic ，那么 MetaData 的 更新
        // 状态为 true
        metadata.add(topic);

        // 根据 cluter 缓存的 Map<String, List<PartitionInfo>> partitionsByTopic 获取 topic 对应的 partition 信息
...
//                System.out.println("metaData Blocking ++++++++++++++++++++++++++++++++++++++++=");
