...
public final class RecordAccumulator {
    ...
    // 正在往缓冲区填充数据的records 数量 ( 还没有填充完毕 )
    private final AtomicInteger appendsInProgress;
    // 容纳的消息大小
    // batch.size  默认: 16384 = 16 * 1024
    ...
    // 每一个 topic 对应一个 Deque，
    // 当 ProducerBatch 缓冲区满了后，会新建一个 ProducerBatch 存放新的数据
    private final ConcurrentMap<TopicPartition, Deque<ProducerBatch>> batches;
    // 成功新建的 producerBatch
    private final IncompleteBatches incomplete;
    // The following variables are only accessed by the sender thread, so we don't need to protect them.
    // producer 顺序性机制，以时间的代价，换取数据的有序性
    /**
     * 1. 将数据写入到 ProducerBatch 内的一个缓冲区中
     * 2. 缓冲区满了、或者时间到了 将获取队列的第一个 producerBatch 放入到 muted，（此时的 ProducerBatch 仍在队列中）
     * 3. 关闭 ProducerBatch 的数据写入通道，并构建一个 producer 请求到 kafka server 端
     *     3.1 在这期间，当有新的数据来是，发现数据写入通道关闭了，会新建一块缓冲区和 ProducerBatch 来存放数据
     *     3.2 当新的ProducerBatch 满足可以发送数据的条件时，由于 muted 中存在该 TopicPartition，导致消息一值存在队列中，等待原来的 producerBatch 发送完毕
     *     3.3 然后一直这么循环下去
     *  4. 该 producerBatch 发送完毕后
     *      4.1 如果需要重新发送的话，在队首新增一个原来的 producerBatch
     *   5. 之后，从 队列中删除 producerBatch 释放内存，
     *   6. 最后 muted 删除掉 producerBatch，等待下一次获取 producerBatch
     */
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        /**
         *
         *  一个 RecordAppendResult 对应 一个 Topicpartition
         *s
         *
         */
/** 直接处理 ***/
        try {
            ...
            synchronized (dq) {
                ...
                // 如果 dq 没有元素，或者 appendStream 被关闭 则返回 null
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, headers, callback, dq);

//                System.out.println("appendResult : " + appendResult);
				// 没有 ProducerBatch , 或者内存空间不足
...
//                System.out.println("appendResult2 : " + appendResult);
                if (appendResult != null) {
                    // Somebody else found us a batch, return the one we waited for! Hopefully this doesn't happen often...
                    return appendResult;
                }

/** 缓冲区空间不足、 appendStream 关闭，或者第一次  ***/
                MemoryRecordsBuilder recordsBuilder = recordsBuilder(buffer, maxUsableMagic);

                //  根据 topic 创建 ProducerBatch
                ProducerBatch batch = new ProducerBatch(tp, recordsBuilder, time.milliseconds());

                // 如果 headers 没有设置，则 null
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            ...
            // 获取 topic 的 leader 节点
            Node leader = cluster.leaderFor(part);

            // 避免读取 deque 数据的时候，Producer 正在发送数据，造成数据获取有误
            synchronized (deque) {
                if (leader == null && !deque.isEmpty()) {
                    // This is a partition for which leader is not known, but messages are available to send.
                    // Note that entries are currently not removed from batches when deque is empty.
                    unknownLeaderTopics.add(part.topic());
                } else if (!readyNodes.contains(leader) && !isMuted(part, nowMs)) { // mute 保证只有一个 ProducerBatch 处于发送的状态
                    ProducerBatch batch = deque.peekFirst();
                    if (batch != null) {
                        long waitedTimeMs = batch.waitedTimeMs(nowMs);
                        boolean backingOff = batch.attempts() > 0 && waitedTimeMs < retryBackoffMs;
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        boolean full = deque.size() > 1 || batch.isFull(); // batch 满了
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            // 获取 topicpartition 对应的 Deque<ProducerBatch>
    ...
    public Map<Integer, List<ProducerBatch>> drain(Cluster cluster, Set<Node> nodes, int maxSize, long now) {
        ...
        // 遍历每一个 Leader 节点
        for (Node node : nodes) {
//            System.out.println("遍历的 node : " + node.toString());
            // 获取 broker 上的需要发送数据的 Topicpartition 对应的第一个 ProducerBatch
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        if (d != null)
            return d;
        d = new ArrayDeque<>();
        // 如果已经存在，则返回 d , 否则返回 null
        // 保证在 多线程的环境下 ， 准确获取  Deque<ProducerBatch>
