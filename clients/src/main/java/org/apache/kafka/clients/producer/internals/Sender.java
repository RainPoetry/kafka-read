...
public class Sender implements Runnable {
    ...
    private long sendProducerData(long now) {
        ...
        // 获取数据发送 topic 的 leader 节点
        RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now);

//        System.out.println(result.readyNodes.toString());
        ...
        /**
         *  server.properties 配置信息，仅在 metadata 第一次更新时采用，
         *      后面的 node 信息是 kafka server 返回回来的数据
         *
         *  当 metadata 第一次发起更新后 ，需要根据 Kafka Server 返回回来的 信息更新 node 信息，
         *     后续 的 metadata 更新，如果 node 无误的话，不会发生改变
         */
//        System.out.println(cluster.nodes().toString());
        while (iter.hasNext()) {
            Node node = iter.next();
        ...
        // 获取 需要向 broker 发送数据的 topicPartition 的首个 ProducerBatch
