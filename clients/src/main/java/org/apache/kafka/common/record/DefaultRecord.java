...
public class DefaultRecord implements Record {
    ...
    public static int writeTo(DataOutputStream out,
                              int offsetDelta,          // offset 相差
                              long timestampDelta,      //  时间差
                              ByteBuffer key,
                              ByteBuffer value,
                              Header[] headers) throws IOException {
        // 计算 相关数据压缩后的大小，采用 varint、varlong 方式九三的
        int sizeInBytes = sizeOfBodyInBytes(offsetDelta, timestampDelta, key, value, headers);
        // 将数据大小压缩后，再存入到 ByteBuffer
    ...
    private static int sizeOfBodyInBytes(int offsetDelta,
        ...
        size += ByteUtils.sizeOfVarint(offsetDelta);  // offset varint 压缩大小
        size += ByteUtils.sizeOfVarlong(timestampDelta);  // 时间差  varlong 压缩大小
