...
public class FileRecords extends AbstractRecords implements Closeable {
    ...
    // mutable state
    private final AtomicInteger size;  // channle.limit - start
    ...
    FileRecords(File file,
                ...
                int start,          //  0
                int end,            // MAX
                boolean isSlice) throws IOException {  //  false
        this.file = file;
    ...
    public LogOffsetPosition searchForOffsetWithSize(long targetOffset, int startingPosition) {
        // 默认：DefaultFileChannelRecordBatch ， 相当于 DefaultRecordBatch 的存在
        // DefaultFileChannelRecordBatch 从 FileChannel 获取数据
        // DefaultRecordBatch 从 ByteBuffer 获取数据
        // 一个 MemoryRecords 对应一个 DefaultFileChannelRecordBatch
        // segment 中有多少个 MemoryRecords 就需要迭代多少次
        for (FileChannelRecordBatch batch : batchesFrom(startingPosition)) {
            // baseoffset + offsetDelta = 最后一个 offset
            long offset = batch.lastOffset();
            // 如果 lastOffset > targetOffset
    ...
    public static class LogOffsetPosition {
        ...
        public final int position;  // 物理位置
