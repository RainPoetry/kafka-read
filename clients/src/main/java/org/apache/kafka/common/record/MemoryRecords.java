...
public class MemoryRecords extends AbstractRecords {
    ...
    //  一个 数据缓存区（ByteBuffer） 对应一个 MutableRecordBatch
    /**
     *
     */
    ...
    public AbstractIterator<MutableRecordBatch> batchIterator() {
    	// RecordBatchIterator 需要获取下一个数据
		// ByteBufferLogInputStream 通过数据源来返回下一个数据
		RecordBatchIterator recordBatchIterator = new RecordBatchIterator<>(new ByteBufferLogInputStream(buffer.duplicate(), Integer.MAX_VALUE));
