...
import static org.apache.kafka.common.utils.Utils.sorted;
...
public class MemoryRecordsBuilder {
    ...
    public MemoryRecordsBuilder(ByteBufferOutputStream bufferStream,
        ...
        // 数据默认的压缩格式： none
        // appendStream 是 bufferStream 数据压缩后的
    ...
    public void close() {
        ...
        if (numRecords == 0L) {
            ...
            // 创建一个与原始缓冲区共享内容的新缓冲区
            // 新缓冲区的内容将为此缓冲区的内容。此缓冲区内容的更改在新缓冲区中是可见的，反之亦然；
            // 这两个缓冲区的位置、界限和标记值是相互独立的。
            ByteBuffer buffer = buffer().duplicate();
            buffer.flip();
            buffer.position(initialPosition);
//            System.out.println("memory  buffer : " + buffer.remaining() + " | " + buffer.position());
    ...
    public boolean hasRoomFor(long timestamp, ByteBuffer key, ByteBuffer value, Header[] headers) {
        // appendStream close 或者 数据写入的空间不足 才为 true
        if (isFull())
            return false;
    ...
    public boolean isFull() {
        ...
        // apendStream 只有close 才是 CLOSED_STREAM，
