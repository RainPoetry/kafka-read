...
public final class ByteUtils {
     ...
     */
    // 0xffffffffL （ L 表示 long ）： 有 L 的话，表示第 32 位不是符号位
    //                                没有 L , 表示第 32 为是符号位
    // 将 Int 强转为 long 类型（符号位参与计算）
     ...
     */
    // unsigned ： 当数据没有负数时，在相同的字节下 ，可以表达更大的范围
    // signed -> unsigned : 可以表示更多的数值 （没有符号位）
    // signed int：	(0x7fffffff)-2147483647 ------ (0x80000000)2147483647  (第一个为符号位，其余 31 个为数值位)
    // unsigned int ： 0 ------ 4294967295 (没有符号位，全部都是数值位)
    // 数据允许范围 0 ~ 4294967295
    // 当值 > 2147483647, value的第32位是数值位，因为强转为 int , 数值位变成符号位，即符号位参与预算
    //   造成生成的数值 是负的
