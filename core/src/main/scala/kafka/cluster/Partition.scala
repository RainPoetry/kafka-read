...
class Partition(val topic: String,
  ...
  // 所有的 replicas ， broker 启动时发送 LeaderAndIsr 请求的时候初始化
  ...
  // leader 副本 ID
  // 如果本地 broker 是该 partition 的副本 leader , id 则为 broker id, 否则 id 为 null
  ...
  def makeLeader(controllerId: Int, partitionStateInfo: LeaderAndIsrRequest.PartitionState, correlationId: Int): Boolean = {
    val (leaderHWIncremented, isNewLeader) = inWriteLock(leaderIsrUpdateLock) {
      ...
      // 本地 replica 是不是 leader, 如果不是的话则为 true
   ...
   *  greater (that is, no updates have been missed), return false to indicate（表明） to the
  ...
  def appendRecordsToLeader(records: MemoryRecords, isFromClient: Boolean, requiredAcks: Int = 0): LogAppendInfo = {
    val (info, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
          // 获取 partition 对应的 Log 对象
          val log = leaderReplica.log.get
          // 默认是  1  min.insync.replicas
          ...
          // 向 TopicPartition 对应的 Log 写入数据 ，并且更新 LEO
          val info = log.appendAsLeader(records, leaderEpoch = this.leaderEpoch, isFromClient)
          // probably unblock some follower fetch requests since log end offset has been updated
          // 如果有 follower 在延迟队列中，则通知他
