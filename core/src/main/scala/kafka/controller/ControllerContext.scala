...
class ControllerContext {
  ...
  // /controller_epoch 节点下的数据
  // 在进行 leader 选举的时候获取的
  var epoch: Int = KafkaController.InitialControllerEpoch
  var epochZkVersion: Int = KafkaController.InitialControllerEpochZkVersion
  // zk 上 /brokers/topics  下的信息
  var allTopics: Set[String] = Set.empty
  // topic 的 partition 和 replica 信息
  private var partitionReplicaAssignmentUnderlying: mutable.Map[String, mutable.Map[Int, Seq[Int]]] = mutable.Map.empty
  // zk 上 /brokers/topics/demo/partitions/0/state (例子) 的信息
  // leader 节点、 isr 信息都在 state 节点中
