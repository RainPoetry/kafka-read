...
class KafkaController(val config: KafkaConfig, zkClient: KafkaZkClient, time: Time, metrics: Metrics, initialBrokerInfo: BrokerInfo,
  ...
  info("kafka controller is start")

  @volatile private var brokerInfo = initialBrokerInfo

  private val stateChangeLogger = new StateChangeLogger(config.brokerId, inControllerContext = true, None)

  // leader 选举完成后进行初始化 (只有 leader 才会)
 ...
 // 分区状态机
  ...
  private def onControllerFailover() {
    ...
    // 对 handler.path 注册 handler 事件 (监听子目录修改)
    childChangeHandlers.foreach(zkClient.registerZNodeChildChangeHandler)
    val nodeChangeHandlers = Seq(preferredReplicaElectionHandler, partitionReassignmentHandler)
    // 对 handler.path 注册 handler 事件 (监听当前目录修改)
    ...
    // 从 zk 中 获取 broker 和 topic 、partiotion 信息缓存到 controllerContext
    ...
    // 向每一个 broker 发送 UpdateMetaDataRequest
  ...
  private def initializeControllerContext() {
    ...
    // 从 zk 上获取 topic 的 leader 和 isr 信息并缓存到 partitionLeadershipInfo
  ...
  private[controller] def sendUpdateMetadataRequest(brokers: Seq[Int], partitions: Set[TopicPartition] = Set.empty[TopicPartition]) {
    try {
      brokerRequestBatch.newBatch()
      // partitions is empty
      // 获取 topic 、 Partition 、 replica 的相关信息并缓存起来，准备发送  UpdateMetadata 请求
      // 以上信息都是 从 controllerContext 中获取的，
      // controllerContext 的信息是从 zk 上获取的
      brokerRequestBatch.addUpdateMetadataRequestForBrokers(brokers, partitions)
      // 向每一个 broker 发送 UpdateMetadataRequest 请求
  ...
  case object Startup extends ControllerEvent {
    ...
    override def process(): Unit = {
      // 给 /controller 节点 注册 controllerChangeHandler 事件
      // default: false
      val flag = zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler)
  ...
  private def elect(): Unit = {
    ...
    try {
      //  创建/controller 临时节点 , /controller_epoch 数据 +1
      // 返回选举成功后的 /controller  和 /controller_epoch 的 dataVersion 信息
...
// 对应 /brokers/topics/demo/partitions/0/state 的信息
