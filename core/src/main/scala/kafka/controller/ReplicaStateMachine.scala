...
class ReplicaStateMachine(config: KafkaConfig,
                          ...
                          replicaState: mutable.Map[PartitionAndReplica, ReplicaState], // 状态机启动的时候赋值
  ...
  def startup() {
    ...
    // 从 controllerContext 中获取 replica 信息到 replicaState 中
  ...
  private def initializeReplicaState() {
    ...
    controllerContext.allPartitions.foreach { partition =>
      // 获取 controllerContext 中 TopicPartition 的 replica 信息
      // 获取该 partition 对应的所有的 replica 信息
    ...
    if (replicas.nonEmpty) {
      try {
        ...
        replicas.groupBy(_.replica).map { case (replicaId, replicas) =>
          ...
          // 将 topicPartition 符合条件的 replica 信息缓存到  leaderAndIsrRequestMap 中的对应的 broker 下面
          doHandleStateChanges(replicaId, partitions, targetState, callbacks)
        }
        // 向 Broker 发送 leaderAndIsrRequest 请求
        ...
        validReplicas.foreach { replica =>
          ...
          replicaState(replica) match {
            ...
            case _ =>
              controllerContext.partitionLeadershipInfo.get(partition) match {
                case Some(leaderIsrAndControllerEpoch) =>
                  // 将 Replica 信息添加到 leaderAndIsrRequestMap 所对应的 broker 区域中
                  controllerBrokerRequestBatch.addLeaderAndIsrRequestForBrokers(Seq(replicaId),
                    replica.topicPartition,
                    leaderIsrAndControllerEpoch,
