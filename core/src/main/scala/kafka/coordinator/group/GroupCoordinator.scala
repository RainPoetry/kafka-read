...
class GroupCoordinator(val brokerId: Int,
  ...
  // 处理加入组请求
  def handleJoinGroup(groupId: String,     // 对配置 KafkaConsumer 时指定
                      memberId: String,   // 默认为 空
                      clientId: String,
                      clientHost: String,
                      rebalanceTimeoutMs: Int,
                      sessionTimeoutMs: Int,
                      protocolType: String,     // default : consumer
      ...
      groupManager.getGroup(groupId) match {
        ...
        case None =>
          if (memberId != JoinGroupRequest.UNKNOWN_MEMBER_ID) {
            ...
            // create group
            // if group not exist then add it to groupMetadataCache
            //   else return the current group
      ...
      if (!group.is(Empty) && (!group.protocolType.contains(protocolType) || !group.supportsProtocols(protocols.map(_._1).toSet))) {
        ...
        group.currentState match {
          ...
          case PreparingRebalance =>
            if (memberId == JoinGroupRequest.UNKNOWN_MEMBER_ID) {
              // 如果没有 memberId， 随机分配一个，然后加入到 group
  ...
  private def addMemberAndRebalance(rebalanceTimeoutMs: Int,
                                    sessionTimeoutMs: Int,
                                    clientId: String,   // Request Header 指定的
                                    clientHost: String,
                                    protocolType: String,
                                    protocols: List[(String, Array[Byte])],
                                    group: GroupMetadata,
  ...
  // 删除没有加入 group 的 member
  // if the member is the leader of the group
  //
