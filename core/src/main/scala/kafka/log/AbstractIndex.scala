...
/**
 * The abstract index class which holds entry format agnostic(不可知) methods.
 *
...
abstract class AbstractIndex[K, V](@volatile var file: File, val baseOffset: Long,
  ...
  /*
   ...
   operations are in page #12, and all the in-sync(同步中) follower / consumer lookups read page #0,6,9,11,12. As these pages
 ...
 */
  ...
  @volatile  // 每添加一个 entry , 那么 + 1
  // 前面进行了设置， mmap.position()  = 可以开始写入数据时的 位置
   ...
   * Lookup lower and upper bounds for the given target.
    *
    * 1. 判断 warnEntry 的 第一个 enrty 的 offset 和 target 的大小
    * 2. offset < target : 在 offset 和 segment 最后一个 entry 的 offset 间进行折半查找
    * 3. target < 0 ： 返回 （-1,0）
    * 4. 否则： 0 ~ offset 间进行这班查找
    *
    * return =  target 所对应的 slot 的范围
    *
    * 返回的值; target 所在的 entry
    *
    * target : 表示查找数据的 offset
   */
  private def indexSlotRangeFor(idx: ByteBuffer, target: Long, searchEntity: IndexSearchEntity): (Int, Int) = {
    ...
    // 通过对 slot 进行二分查找，获取 target 所在的 slot
    def binarySearch(begin: Int, end: Int) : (Int, Int) = {
      ...
      while(lo < hi) {
        ...
        // mid 所在 entry 的 offset 和 target 的大小比较
    ...
    // warmEntries 表示 segment 中最后 8192 / entrySize 个 enrty
   // 获取 warmEntries 中的第一个 enrty
    val firstHotEntry = Math.max(0, _entries - 1 - _warmEntries)
    // check if the target offset is in the warm section of the index
    // parseEntry(idx, firstHotEntry) 返回的是 firstHotEntry 的 offset 和 position
    // firstHotEntry 的 offset  <  target ?
