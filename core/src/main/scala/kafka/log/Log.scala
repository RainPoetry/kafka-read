 ...
 */
case class LogAppendInfo(var firstOffset: Option[Long],   // 数据的起始 offset
                         var lastOffset: Long,          //  数据末尾的 offset
                         var maxTimestamp: Long,
                         var offsetOfMaxTimestamp: Long,
                         var logAppendTime: Long,
                         var logStartOffset: Long,  // default 0
 ...
 *
 * The log is a sequence of LogSegments, each with a base offset denoting(表示) the first message in the segment.
 *
 ...
 */
@threadsafe
class Log(@volatile var dir: File,       // topicPartition 数据目录（demo_0）
          @volatile var config: LogConfig,   // topic 的配置信息
          @volatile var logStartOffset: Long,  // log-start-offset-checkpoint 中的 topic 对应的值，默认0
          @volatile var recoveryPoint: Long,  // recovery-point-offset-checkpoint 中 topic 对应的值
          scheduler: Scheduler,         // KafkaScheduler ， 用于调度任务
          brokerTopicStats: BrokerTopicStats,
          val time: Time,               //  创建 Log 实例对象时刻的时间
  ...
  // next Offset =  最后一个 #{MemoryRecords}# 的 nextOffset
   ...
   * temporary abuse seems justifiable(合理的) and saves us from scanning the log after deletion to find the first offsets
  ...
  /* Keep track of(不要跟丢) the current high watermark in order to ensure that segments containing offsets at or above it are
   * not eligible(合格) for deletion(缺失). This means that the active segment is only eligible for deletion if the high watermark
  ...
  // 在 locally 加载 segment 文件 ,
  // Long = 从文件名获取的offset，LogSegment 对应 log 文件
  ...
  locally {
    ...
    // 加载 log 中的所有 segment ，并获取 log 的 next Offset
    // nextOffset =  0 ( 在没有错误文件的时候 <-delete 文件>)
    val nextOffset = loadSegments()
//    println("nextOffset : " + nextOffset +" | segments : " + segments.toString)
      ...
      if (isIndexFile(file)) {
        ...
        try segment.sanityCheck(timeIndexFileNewlyCreated)
          ...
          case e: CorruptIndexException =>
            ...
            recoverSegment(segment)  //  index 和 timeindex 文件重新写入
        }
        addSegment(segment)
  ...
  private def loadSegments(): Long = {
    ...
    retryOnOffsetOverflow {
      ...
      segments.clear()
      // 一个 log 对应 一个 segmentFile
      // 采用 RandonAccessFile 和 FileChannel 形式加载 log 文件
      // 加载 index 、 timmeindex  文件 映射到 虚拟内存
      // 对  index 和 timmeindex 文件数据重新写入  , 然后生成一个 snapshot 文件
      loadSegmentFiles()
    }
    ...
    if (logSegments.isEmpty) {
      ...
      // 最后一个 MemoryRecords 的 nextOffset
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          ...
          val validateAndOffsetAssignResult = try {
            // 校验 records 并给 records 分配 offset
   ...
   * <ol>
   * <li> each message matches its CRC  crc 检验
   * <li> each message size is valid    数据检验
   * <li> that the sequence numbers of the incoming record batches are consistent(一致) with the existing state and with each other.
    *
   * </ol>
   *
   * Also compute the following quantities:
   * <ol>
   * <li> First offset in the message set       message 的第一个 offset
   * <li> Last offset in the message set        message 的最后一个 offset
   * <li> Number of messages                    message 的个数
   * <li> Number of valid bytes                 有效的字节数
   * <li> Whether the offsets are monotonically(单调) increasing
   * <li> Whether any compression codec is used (if many are used, then the last one is given)  是否有压缩处理
   * </ol>
   */
  ...
  /**
   * Cleanup old producer snapshots after the recovery point is checkpointed. It is useful to retain(保留)
...
object Log {
  ...
  def apply(dir: File,            // topicPartition 数据目录（demo_0）
            config: LogConfig,    // topic 的配置信息
            logStartOffset: Long, // log-start-offset-checkpoint 中的 topic 对应的值，默认0
            recoveryPoint: Long,  // recovery-point-offset-checkpoint 中 topic 对应的值
            scheduler: Scheduler,  // KafkaScheduler ， 用于调度任务
            brokerTopicStats: BrokerTopicStats,
            time: Time = Time.SYSTEM,  //  现在时刻
            maxProducerIdExpirationMs: Int,
            producerIdExpirationCheckIntervalMs: Int,
            logDirFailureChannel: LogDirFailureChannel): Log = {
    // 通过 目录名 获取 topic + partition
