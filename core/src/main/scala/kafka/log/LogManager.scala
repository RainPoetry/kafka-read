...
/**
  ...
  */
@threadsafe
class LogManager(logDirs: Seq[File], // 对应配置文件的 log.dirs
                 initialOfflineDirs: Seq[File],
                 val topicConfigs: Map[String, LogConfig], // note that this doesn't get updated after creation
                 val initialDefaultConfig: LogConfig,
                 val cleanerConfig: CleanerConfig,
                 recoveryThreadsPerDataDir: Int,          // 每一个 数据目录的线程个数
                 val flushCheckMs: Long,
                 val flushRecoveryOffsetCheckpointMs: Long,  // recovery-offset 的循环周期
                 val flushStartOffsetCheckpointMs: Long,     // start-offset 的循环周期
  ...
  // 分区与日志实例的对应关系 - 在给每一个分区指定 Log 实例时在制定
  ...
  // recovery-point-offset-checkpoint ，不存在则创建
  //  每个数据目录都有一个检查点文件,存储这个数据目录下所有分区的检查点信息
  // 一个目录对应一个 OffsetCheckpointFile
  @volatile private var recoveryPointCheckpoints = liveLogDirs.map(dir =>
    (dir, new OffsetCheckpointFile(new File(dir, RecoveryPointCheckpointFile), logDirFailureChannel))).toMap
  // log-start-offset-checkpoint 不存在则创建
  // 一个 目录 对应一个 OffsetCheckpointFile (个人感觉没卵用，也许是因为版本的问题)
  ...
  private def loadLog(logDir: File, recoveryPoints: Map[TopicPartition, Long], logStartOffsets: Map[TopicPartition, Long]): Unit = {
    ...
    // 通过文件名来获取 topic + partition
    val topicPartition = Log.parseTopicPartitionName(logDir)
    // 获取该 topicPartition 的配置
  ...
  = {
    ...
    for (dir <- liveLogDirs) { //  log.dirs 下的每一个目录
      try {
        ...
        try {
          /**
            ...
            * 第一行： version（文件的版本）
            * 第二行：entrySize
            * （recoveryPoints 中的是数据）后面的行：entry ： topic partition offset (TopicPartition-offset)
            * 当有数据改动时，生成新的文件，然后替换
            */
        ...
        /**
          ...
          *  yield   ->    值保存在该区域中
          */
          //  jobsForDir的个数 = kafka 目录下的分区个数
        val jobsForDir = for {
          dirContent <- Option(dir.listFiles).toList
          logDir <- dirContent if logDir.isDirectory  // 如果文件是分区的话（是否为文件夹来判断）
        } yield {
          CoreUtils.runnable {
            try {
              // 处理日志目录下的每一个 topic 分区目录
              // 每一个分区 对应 一个 Log , 然后对 log 分类：(根据文件后缀)
              // delete（-delete）/  future (-future) / current（没有后缀）
              // 只是定义执行的语句
  ...
  def startup() {
    ...
    if (scheduler != null) {
      ...
      scheduler.schedule("kafka-log-flusher",
        ...
        period = flushCheckMs,
        TimeUnit.MILLISECONDS)
      // 更新 recovery-point-offset-checkpoint 文件，并删除 在 check point 之后的 snapshot
  ...
  = {
    for {
      ...
      // OffsetCheckpointFile 文件
      checkpoint <- recoveryPointCheckpoints.get(dir)
    } {
      try {
        // 将 TopicPartition 信息写入到 临时文件， 替换掉 原来的  OffsetCheckpointFile文件
        checkpoint.write(partitionToLog.mapValues(_.recoveryPoint))
        // 删除 checkpoint 之后的 snapshot
...
object LogManager {
  ...
  def apply(config: KafkaConfig,
    ...
    // 从 kafkaConfig 中获取 log 配置信息
    val defaultProps = KafkaServer.copyKafkaConfigToLog(config)
    val defaultLogConfig = LogConfig(defaultProps)

    // read the log configurations from zookeeper
    // zk Path ： /brokers/topics
    // topicConfigs ： topics 在 zk 中的相关信息
    // failed ： 信息获取失败的 topic
