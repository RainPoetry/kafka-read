 ...
 */
@nonthreadsafe
class LogSegment private[log] (val log: FileRecords,          // log 文件 （RandomAccessFile + FileChannel）
                               val offsetIndex: OffsetIndex,  // index 文件
                               val timeIndex: TimeIndex,       // timeIndex  文件
                               val txnIndex: TransactionIndex,
                               val baseOffset: Long,            // 从文件名获取到的
                               val indexIntervalBytes: Int,     // 控制 index 文件多少数据就添加一行索引
  ...
  private[log] def translateOffset(offset: Long, startingFilePosition: Int = 0): LogOffsetPosition = {
    // 查找 index 文件的 slot , slot.offset >= targetOffset
    val mapping = offsetIndex.lookup(offset)
   // 从 slot.position 位置开始查找 log 文件
    // 找到 MemoryRecords.lastoffset >= targetOffset
    // 返回 lastOffset , slot.position
  ...
  /**
    * 思路：
       1. targetOffset >= find_slot.offset 因此，targetOffset.position >= find_slot.position
            所以 需要到 log 文件进行迭代查找
    *  3. targetOffset 的处理：
    *     a. 得到 index 文件 的 slot
    *     b. 确定 slot 的 position
    *     c. 从 position 开始遍历 log 文件，获取到 lastOffset >= position 的 MemoryRecords 的 position
    *     (当 tragetOffset 不是 index slot 中的 position 时，就需要这么处理)
    *     d. 从 FileChannel 获取 MemoryRecords.postion ~ log.sizeInBytes-MemoryRecords.position 范围的数据（n 个 records）
    *
    */
  @threadsafe
  def read(startOffset: Long, maxOffset: Option[Long], maxSize: Int, maxPosition: Long = size,
    ...
    // 1. 根据 targetOffset 找到 index 文件所属的 slot（如果超出最大范围，则是最后一个 slot）
    // 2.  根据 slot 计算出 offset 和 position
    // 3. 从 startOffset 开始，迭代 segment 中的所有的 MemoryRecords
    // 4. 返回匹配的 MemoryRecords 的 lastOffset 和 Position ： lastOffset >= targetOffset
    val startOffsetAndSize = translateOffset(startOffset)
    /*if (startOffsetAndSize!=null)
      println(s"startOffsetAndSize: ${startOffsetAndSize.offset}-${startOffsetAndSize.position}")*/

    // if the start position is already off the end of the log, return null
    if (startOffsetAndSize == null)
      return null

    //  index 文件中 的 position
    val startPosition = startOffsetAndSize.position
    // startOffset: 需要查找的 offset
    // startPosition : startOffset 对应的 slot.position
    ...
    // 从 startPosition 位置开始，截取 fetchSize 大小的数据
   ...
   * from the end of the log and index.
    *
    * 通过 log 文件对 index 和 timeindex 文件进行重新写入
   *
  ...
  def readNextOffset: Long = {
    //  index 文件 ： targetOffset -> position （对 slot 进行二分查找）
    //  log 文件 ： 从 position 位置开始， MemoryRecords.lastoffset >= targetOffset , 获取其 position
    //  获取 Records : fileChannle 从 position 开始，截取 log.sizeInBytes - position 大小的数据
...
object LogSegment {
  ...
  // baseOffset 从 文件名获取
  // cofig  topicPartition 对应的 config
  def open(dir: File, baseOffset: Long, config: LogConfig, time: Time, fileAlreadyExists: Boolean = false,
           initFileSize: Int = 0, preallocate: Boolean = false, fileSuffix: String = ""): LogSegment = {
    val maxIndexSize = config.maxIndexSize
    new LogSegment(
      //  .log 文件 ： 一个 segment 文件对应一个 FileChannel
      FileRecords.open(Log.logFile(dir, baseOffset, fileSuffix), fileAlreadyExists, initFileSize, preallocate),
      // .index 文件  =  offset + position = 4 bytes + 4 bytes
