...
//  println("file : %s , entries: %s  , offset : %s ".format(file.getAbsolutePath,_entries,lastEntry.offset))
  ...
  def lookup(targetOffset: Long): OffsetPosition = {
    maybeLock(lock) {
      ...
      // 获取 targetOffset 对应的 slot 边界范围的下界
      // targetoffset 一般是位于两个 slot offset 之间，取 offset 较小的 slot
  ...
  // 获取 bytebuffer 中第 n 个 entry 的 offset
  private def relativeOffset(buffer: ByteBuffer, n: Int): Int = buffer.getInt(n * entrySize)

  private def physical(buffer: ByteBuffer, n: Int): Int = buffer.getInt(n * entrySize + 4)


  // n : slot  -  表示第几个 message
  override def parseEntry(buffer: ByteBuffer, n: Int): IndexEntry = {
    // key : 前 4 bytes    -  offset
    // value： 后 4 bytes  -  position
