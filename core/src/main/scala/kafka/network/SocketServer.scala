...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time, val credentialProvider: CredentialProvider) extends Logging with KafkaMetricsGroup {
   ...
   * to ensure that all credentials(证书、证件) have been loaded before authentications are performed.
   * Acceptors are always started during `startup` so that the bound port is known when this
   * method completes even when ephemeral(临时的) ports are used. Incoming connections on this server
  ...
  private def createAcceptorAndProcessors(processorsPerListener: Int,
    ...
    endpoints.foreach { endpoint =>
      ...
      // 通过正则表达式 提取 listeners 中的 PLAINTEXT 得以实现
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  // 创建 ServerSocketChannel , 用于 接收 SocketChannel 的链接请求
   ...
   */
  def run() {
    // 注册 ServerSocketChannel 到 Selector , 并注册 OP_ACCEPT 事件
  ...
  def accept(key: SelectionKey, processor: Processor) {
    // 注册到 Selector 上的 ServerSocketChannel
    val serverSocketChannel = key.channel().asInstanceOf[ServerSocketChannel]
    // 获取待 accept 的 SocketChannel
...
private[kafka] class Processor(val id: Int,
  ...
  // 创建 selector , 也就是 client 端的 Selector( 对 nio selector 封装了一层)
    ...
    selector.completedReceives.asScala.foreach { receive =>
      try {
        openOrClosingChannel(receive.source) match {
          case Some(channel) =>
            // 通过 apikeys 和 version 获取相应的 schema , 通过该 schema 来进行解析
