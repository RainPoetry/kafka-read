...
abstract class AbstractFetcherThread(name: String,
  ...
  // partition 状态信息
  // 管理者 fetcher 线程有拥有的 topicpartiition
  // 一个 fetcher 线程负责多个 partition 的 fetcher 请求
  ...
  /**
    * - Build a leader epoch fetch based on partitions that are in the Truncating phase(阶段)
  ...
  private def maybeTruncate(): Unit = {
    ...
    if (epochRequests.nonEmpty) {
      // 发送 LIST_OFFSETS 请求， 和获取 leader epoch 和 lastOffset
