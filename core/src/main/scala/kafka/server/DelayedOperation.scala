...
abstract class DelayedOperation(override val delayMs: Long,
  ...
  /**
   * Thread-safe variant(变种) of tryComplete() that attempts completion only if the lock can be acquired
   * without blocking.
   *
...
final class DelayedOperationPurgatory[T <: DelayedOperation](purgatoryName: String,
  ...
  def tryCompleteElseWatch(operation: T, watchKeys: Seq[Any]): Boolean = {
    ...
    // The cost of tryComplete() is typically proportional(按比例) to the number of keys. Calling
    ...
    for(key <- watchKeys) {
      ...
      // 将 operation 加入到 key 的 Watch 列表中
