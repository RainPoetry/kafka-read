...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleLeaderAndIsrRequest(request: RequestChannel.Request) {
    ...
    // We can't have the ensureTopicExists check here since the controller sends it as an advisory(咨询) to all brokers so they
  ...
  def handleProduceRequest(request: RequestChannel.Request) {
    ...
    if (authorizedRequestInfo.isEmpty)
      ...
      // 对于 topic  信息无误且认证成功的 produceRequest 才追加 Record
  ...
  private def getOrCreateInternalTopic(topic: String, listenerName: ListenerName): MetadataResponse.TopicMetadata = {
    ...
    println(metadataCache.getAllTopics())
  ...
  // 获取 consumer 的 coordinator 节点
  // __consumer_offsets 主题所对应分区的 leader 节点
    ...
    if (findCoordinatorRequest.coordinatorType == FindCoordinatorRequest.CoordinatorType.GROUP &&
      ...
      val (partition, topicMetadata) = findCoordinatorRequest.coordinatorType match {
        case FindCoordinatorRequest.CoordinatorType.GROUP =>
          // coordinatorKey = group.id
          // partition = Utils.abs(groupId.hashCode) % groupMetadataTopicPartitionCount
          // groupMetadataTopicPartitionCount = __consumer_offsets 的个数
          // partition = groupId.hashCode() % 50;
          val partition = groupCoordinator.partitionFor(findCoordinatorRequest.coordinatorKey)
          // 获取或者创建内部 topic :  __consumer_offsets
          // 返回 topic 的 MetaData
      ...
      def createResponse(requestThrottleMs: Int): AbstractResponse = {
        val responseBody = if (topicMetadata.error != Errors.NONE) {
          ...
          // 返回  __consumer_offsets 主题 所在 分区的 leader 节点
  ...
  def handleJoinGroupRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(request.session, Read, Resource(Group, joinGroupRequest.groupId(), LITERAL))) {
      ...
      // protocol.metadata 只有 topic 信息 (TOPICS_KEY_NAME)
      ...
      groupCoordinator.handleJoinGroup(
        ...
        joinGroupRequest.memberId,  // 默认为 空
        request.header.clientId,
        request.session.clientAddress.toString,
        joinGroupRequest.rebalanceTimeout,
        joinGroupRequest.sessionTimeout,
        joinGroupRequest.protocolType,  // default : consumer
        protocols,
        sendResponseCallback)
    }
  }
