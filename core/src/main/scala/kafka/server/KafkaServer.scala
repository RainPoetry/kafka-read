...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  def startup() {
    try {
      ...
      if (canStartup) {
        ...
        /* setup zookeeper */
        //  创建 kafka 在 zk 中的数据存储目录（if need）
        initZkClient(time)

        /* Get or create cluster_id */
        // zkClient 在 initZkClient 中创建
        // 获取 或者 生成 Cluster Id
        // zk 上的存储节点： /cluster/id
        _clusterId = getOrGenerateClusterId(zkClient)
        info(s"Cluster ID = $clusterId")

        /* generate brokerId */
        // brokerId : server.properties 配置的
        // initialOfflineDirs： log.dirs 中 meta.properties 文件错误的目录
        ...
        // 从 zk 上获取配置信息 来更新 kafkaConfig 配置信息
        config.dynamicConfig.initialize(zkClient)

        /* start scheduler */
        //  封装了schedulerThreadId，用于 定时调度任务
        ...
        /* start log manager */
        // 从 zk、 中加载 topic 相关的信息, 作为 log 的配置信息
        // 读取 log-start-offset-checkpoint 和 recovery-point-offset-checkpoint
        // index和 timeIndex 文件 :  MappedByteBuffer
        // log 文件  :  RandomAccessFile + FileChannel
        // 一个 broker -> 一个 LogManager -> 多个 分区
        // 开启 numRecoveryThreadsPerDataDir 个线程，加载 所有的分区信息
        //
        // 一个线程 -> 一个 分区 -> 一个 Log
        // 一个 Log -> 多个 LogSegment(存放在 Log 的 segments 中)
        ...
        // 开启 Acceptor 线程，接收客户端请求
        socketServer.startup(startupProcessors = false)

        /* start replica manager */
        // 用于 副本管理
        // 从 replication-offset-checkpoint 中读取数据到 highWatermarkCheckpoints
        replicaManager = createReplicaManager(isShuttingDown)
        replicaManager.startup()

        // broker 信息
        val brokerInfo = createBrokerInfo
        // 将 broker信息 注册到 /brokers/ids/brokerId 中
        zkClient.registerBroker(brokerInfo)

        // Now that the broker id is successfully registered, checkpoint it
        // 将 broker 信息写入 metaData
        ...
        // leader 选举
        // leader 节点操作：
        //      从 zk 获取 topic 和 broker 数据到 controllerContext
        //      根据 controllerContext 数据发送 metaDataRequest 到每一台 broker
        //      启动 副本状态机: 根据 zk 上的 replicas 信息 发送 LeaderAndIsrRequest 请求
        //      启动 分区状态机
        //      启动 controller scheduler 线程
        ...
        // 启动  num.io.threads 个数线程，处理 缓存在阻塞队列中的请求 #socketServer.requestChannel#
        //
  ...
  private def getBrokerIdAndOfflineDirs: (Int, Seq[String]) = {
    ...
    // log.dirs: server.properties 配置的
    for (logDir <- config.logDirs) {
      try {
        // 读取 meta.properties 中 的信息
