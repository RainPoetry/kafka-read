...
class ReplicaManager(val config: KafkaConfig,
  ...
  // 该 broker 上存放所有的 partiton 信息
  // 根据 zk 上的 topic 信息发送 leaderAndISR ，将 replica 信息添加进来
  ...
  /**
   * This function periodically runs to see if ISR needs to be propagated（传播）. It propagates ISR when:
   * 1. There is ISR change not propagated yet.
   * 2. There is no ISR Change in the last five seconds, or it has been more than 60 seconds since the last ISR propagation.
   * This allows an occasional ISR change to be propagated within a few seconds, and avoids overwhelming（压倒性） controller and
  ...
  def getPartitionAndLeaderReplicaIfLocal(topicPartition: TopicPartition): (Partition, Replica) =  {
    // 通过 topicPartition 获取 partition
  ...
  def  appendRecords(timeout: Long,
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      // 将 request 数据添加到本地 log 中
      // TopicPartition -> LogAppendResult(log扩展的相关信息)
  ...
  // LeaderAndIsrRequest 实际处理的部分
  // replica 变成 leader or follower
  def becomeLeaderOrFollower(correlationId: Int,
    ...
    replicaStateChangeLock synchronized {
      //  验证 controller 的 epoch，如果是来自旧的 controller，就拒绝这个请求
      if (leaderAndIsrRequest.controllerEpoch < controllerEpoch) {
        ...
        leaderAndIsrRequest.partitionStates.asScala.foreach { case (topicPartition, stateInfo) =>
          // 添加/获取 ReplicaManager 的 Partition 信息
   ...
   *    更新缓存中的 partition metadata
   * 3. Add these partitions to the leader partitions set
   *    将这些 partition 添加到 leader partition 集合中
   *
  ...
  private def makeLeaders(controllerId: Int,
    ...
    try {
      ...
      // 将这些 partition 所对应的信息从 fetcher 线程 中删掉
      replicaFetcherManager.removeFetcherForPartitions(partitionState.keySet.map(_.topicPartition))
      // Update the partition information to be the leader
      // 更新 partition 的 meta 数据
      partitionState.foreach{ case (partition, partitionStateInfo) =>
        try {
          // replica 的 leader 发生改变 ，则为 true
   ...
   *    将 replica 标记为 follower(避免不会动 producer 端接收数据)
   * 3. Stop fetchers for these partitions so that no more data can be added by the replica fetcher threads.
   *    停止 fetcher 线程
   * 4. Truncate the log and checkpoint offsets for these partitions.
   *    裁剪 log 和 checkpoint offset
   * 5. Clear the produce and fetch requests in the purgatory
   *    清除 produce 和 fetch 的延迟操作
   * 6. If the broker is not shutting down, add the fetcher to the new leaders.
   *    向新的 leader 发送 fetcher 请求
   *
