...
private[timer] class TimerTaskList(taskCounter: AtomicInteger) extends Delayed {
  ...
  // TimerTaskList forms a doubly linked cyclic list using a dummy(假) root entry
  ...
  /**
    ...
    *                                 node.prev -> root
    *
    *   TimerTask 插入到队列只需要改变3个 Node 的指针指向情况即可
    */
    ...
    while (!done) {
      ...
      synchronized {
        timerTaskEntry.synchronized {
          // 说明当前 timerTask 还没有加入到 timerTaskList
  ...
  // 删除所有的 TimerTaskEntry, 并对 TimerTaskEntry 执行传入的 func 操作
