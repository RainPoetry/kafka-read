 ...
 * the n-th bucket for [u * (n -1), u * n). Every interval of time unit u, the timer ticks(计时器滴答) and
 ...
 * It is caller's responsibility to enforce it. Simultaneous(同时) add calls are thread-safe.
 */
@nonthreadsafe
private[timer] class TimingWheel(tickMs: Long, wheelSize: Int, startMs: Long, taskCounter: AtomicInteger, queue: DelayQueue[TimerTaskList]) {
  ...
  // 双重检验对于单例模式来说存在风险，因此采用 volatile 进行修饰
  // timer request is out of this interval
  ...
  // 如果 过期时间 > 当前的 timeWheel 周期 + current
  //      则进入一下时间轮
  def add(timerTaskEntry: TimerTaskEntry): Boolean = {

    // TimerTask 的过期时间
    val expiration = timerTaskEntry.expirationMs
//    println(s"expiration: ${expiration} currentTime: ${currentTime}")
    if (timerTaskEntry.cancelled) {
      ...
      if (bucket.setExpiration(virtualId * tickMs)) {
        // The bucket needs to be enqueued(排队的) because it was an expired bucket
  ...
  // 更新 当前时间轮 以及父级时间轮的 currentTime
  def advanceClock(timeMs: Long): Unit = {

    // timeMs 没有过期
    if (timeMs >= currentTime + tickMs) {
      currentTime = timeMs - (timeMs % tickMs)
//      println(s"advance clock- timeMs: ${timeMs} currentTime:${currentTime}")
