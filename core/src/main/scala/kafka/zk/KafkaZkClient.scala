...
class KafkaZkClient private (zooKeeperClient: ZooKeeperClient, isSecure: Boolean, time: Time) extends AutoCloseable with
  ...
  def registerControllerAndIncrementControllerEpoch(controllerId: Int): (Int, Int) = {
    ...
    val (curEpoch, curEpochZkVersion) = getControllerEpoch
      // 1 : controller_epoch 存储的值
      // 2 ：Stat.version( zookeeper 元数据)
      .map(e => (e._1, e._2.getVersion))
      // 如果不存在则创建新的 /controller_epoch
    ...
    // 选举失败处理，检测 /controller 和 /controller_epoch 信息是否正确
    def checkControllerAndEpoch(): (Int, Int) = {
      ...
      if (controllerId == curControllerId) {
        ...
        // 验证选举成功 :  /controller_epoch 的数据 成功+1
  ...
  def getTopicPartitionStatesRaw(partitions: Seq[TopicPartition]): Seq[GetDataResponse] = {
    val getDataRequests = partitions.map { partition =>
      // 获取 brokers/topics/demo/partitions/0/state 的信息
  ...
  def getAllBrokersInCluster: Seq[Broker] = {
    //  获取 /brokers/ids 下的 broker 节点
    val brokerIds = getSortedBrokerList
    // 获取 /brokers/ids/broker 的信息
   ...
   * Gets the assignments for the given topics.
    *   获取 topic 的 partition he1 replica 信息
...
object KafkaZkClient {
  ...
  def apply(connectString: String,
    ...
    // 分别对应 zookeeper.connect、zookeeper.session.timeout.ms、zookeeper.connection.timeout.ms
